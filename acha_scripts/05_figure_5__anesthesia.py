...
from tools import filter_bandpass, mask_to_intervals
import scipy.ndimage as ndi
import scipy.signal as ss
from methods import WaveletQuantileNormalization
...
# Read EEG recording
meta, eeg = read_recording("resources/data/vitaldb/78.h5")
...
freq = 128  # all recordings in the VitalDB dataset are at 128 Hz
artifacts = power >= threshold
artifacts = ndi.binary_closing(artifacts, np.ones(freq))
artifacts = ndi.binary_opening(artifacts, np.ones(freq // 4))
artifacts = ndi.binary_dilation(artifacts, np.ones(freq))


# Artifacted intervals
intervals = mask_to_intervals(artifacts)
...
_art_intervals = mask_to_intervals(
...
fig.savefig("./output/acha_fig5_anesthesia.pdf")
print("Figure saved to `./output/acha_fig5_anesthesia.pdf`.")
